#!/usr/bin/env Rscript
# Purpose:
#  - Compute threshold_25_v2 target-recovery/Jaccard/gene/difficulty outputs
#  - Build threshold_25_v1 vs threshold_25_v2 comparison table
#  - Optionally include threshold_25_uniform if available
...
v1_label <- Sys.getenv("V1_LABEL", "threshold_25")
v2_label <- Sys.getenv("V2_LABEL", "threshold_25_v2")
uniform_label <- Sys.getenv("UNIFORM_LABEL", "threshold_25_uniform")

v1_stage1_rda <- Sys.getenv("V1_STAGE1_RDA", here("results", "dsen_results", v1_label, "dsen_production_results.Rda"))
v1_bootstrap_rda <- Sys.getenv("V1_BOOTSTRAP_RDA", here("results", "dsen_results", v1_label, "dsen_bootstrap_results.Rda"))

v2_stage1_rda <- Sys.getenv("V2_STAGE1_RDA", here("results", "dsen_results", v2_label, "dsen_production_results.Rda"))
v2_bootstrap_rda <- Sys.getenv("V2_BOOTSTRAP_RDA", here("results", "dsen_results", v2_label, "dsen_bootstrap_results.Rda"))

uniform_stage1_rda <- Sys.getenv("UNIFORM_STAGE1_RDA", here("results", "dsen_results", uniform_label, "dsen_production_results.Rda"))
uniform_bootstrap_rda <- Sys.getenv("UNIFORM_BOOTSTRAP_RDA", here("results", "dsen_results", uniform_label, "dsen_bootstrap_results.Rda"))

out_v2_table_dir <- Sys.getenv("OUT_V2_TABLE_DIR", here("results", "tables", v2_label))
out_comparison_csv <- Sys.getenv("OUT_COMPARISON_CSV", here("results", "tables", "threshold_comparison_25_v1_v2.csv"))
out_v2_recovery_rda <- Sys.getenv("OUT_V2_RECOVERY_RDA", here("results", "dsen_results", v2_label, "tissue_target_recovery.Rda"))

out_v2_recovery_csv <- file.path(out_v2_table_dir, "target_recovery_extended.csv")
out_v2_recovery_summary_csv <- file.path(out_v2_table_dir, "target_recovery_extended_summary.csv")
out_v2_jaccard_csv <- file.path(out_v2_table_dir, "jaccard_en_dsen.csv")
out_v2_jaccard_per_drug_csv <- file.path(out_v2_table_dir, "jaccard_en_dsen_per_drug.csv")
out_v2_difficulty_csv <- file.path(out_v2_table_dir, "difficulty_quartile.csv")
...
if (!is.finite(jaccard_threshold) || jaccard_threshold < 0 || jaccard_threshold > 1) {
  stop("JACCARD_THRESHOLD must be in [0,1].")
}

dir.create(out_v2_table_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(dirname(out_comparison_csv), recursive = TRUE, showWarnings = FALSE)
dir.create(dirname(out_v2_recovery_rda), recursive = TRUE, showWarnings = FALSE)

required_stage1 <- c(v1_stage1_rda, v2_stage1_rda)
missing_stage1 <- required_stage1[!file.exists(required_stage1)]
if (length(missing_stage1) > 0) {
  stop("Missing required Stage 1 input files:\n", paste(" - ", missing_stage1, collapse = "\n"))
}

if (!file.exists(v2_bootstrap_rda)) {
  stop("Missing required v2 bootstrap file: ", v2_bootstrap_rda)
}

include_uniform_flag <- toupper(Sys.getenv("INCLUDE_UNIFORM", "AUTO"))
if (!include_uniform_flag %in% c("AUTO", "TRUE", "FALSE")) {
  stop("INCLUDE_UNIFORM must be one of AUTO, TRUE, FALSE.")
}

if (include_uniform_flag == "TRUE") {
  include_uniform <- TRUE
} else if (include_uniform_flag == "FALSE") {
  include_uniform <- FALSE
} else {
  include_uniform <- nzchar(uniform_stage1_rda) && file.exists(uniform_stage1_rda)
}

if (include_uniform && !file.exists(uniform_stage1_rda)) {
  stop("INCLUDE_UNIFORM=TRUE but missing uniform Stage 1 input file: ", uniform_stage1_rda)
}
...
load_stage1_df <- function(rda_file) {
  ...
  if (!"alpha_dsen" %in% names(df)) {
    df$alpha_dsen <- NA_real_
  }
  if (!"pct_improvement" %in% names(df) && all(c("mse_en", "mse_dsen") %in% names(df))) {
    df$pct_improvement <- (df$mse_en - df$mse_dsen) / df$mse_en * 100
  }

  required <- c("drug", "status", "n_samples", "n_tissues", "mse_en", "mse_dsen", "pct_improvement", "alpha_dsen", "lamrat_dsen")
...
load_bootstrap_results <- function(rda_file) {
  if (!file.exists(rda_file)) return(NULL)
...
compute_recovery <- function(bootstrap_results, targets_known, feature_info, thresholds) {
  if (is.null(bootstrap_results)) {
    return(list(per_drug = tibble(), summary = tibble()))
  }
  ...
  recovery <- map_dfr(thresholds, function(threshold) {
    map_dfr(bootstrap_results, function(res) {
      ...
      tibble(
        ...
        en_any = length(intersect(drug_targets, gene_sets$en_genes)) > 0,
        dsen_shared_any = length(intersect(drug_targets, gene_sets$dsen_shared_genes)) > 0,
        dsen_tissue_any = length(intersect(drug_targets, gene_sets$dsen_tissue_genes)) > 0,
        dsen_combined_any = length(intersect(drug_targets, gene_sets$dsen_combined_genes)) > 0
      )
    })
  })
...
compute_jaccard <- function(bootstrap_results, feature_info, threshold) {
  if (is.null(bootstrap_results)) {
    return(list(per_drug = tibble(), summary = tibble(metric = character(0), value = numeric(0))))
  }
  ...
  if (nrow(per_drug) == 0) {
    ...
    return(list(per_drug = per_drug, summary = summary_tbl))
  }
...
compute_lamrat_stats <- function(stage1_df) {
  ...
  list(
    ...
    pct_eq0125 = 100 * mean(abs(vals - 0.125) < 1e-12),
    distribution = dist_str
  )
}

compute_alpha_stats <- function(stage1_df) {
  vals <- stage1_df %>%
    filter(status == "success") %>%
    pull(alpha_dsen)
  vals <- vals[is.finite(vals)]
  ...
  list(
    pct_lt0_2 = 100 * mean(vals < 0.2),
    pct_eq0_05 = 100 * mean(abs(vals - 0.05) < 1e-12),
    pct_eq1 = 100 * mean(abs(vals - 1.0) < 1e-12)
  )
}

lookup_metric <- function(tbl, metric) {
  out <- tbl %>% filter(.data$metric == .env$metric) %>% pull(value)
  if (length(out) == 0) return(NA_real_)
  as.numeric(out[1])
}

collect_metrics <- function(stage1_df, recovery_summary = NULL, jaccard_summary = NULL, difficulty_tbl = NULL) {
  ...
  if (!is.null(difficulty_tbl) && nrow(difficulty_tbl) > 0) {
  ...
  alpha_stats <- compute_alpha_stats(stage1_df)
  ...
  list(
    ...
    alpha_lt_0_2_pct = alpha_stats$pct_lt0_2,
    alpha_eq_0_05_pct = alpha_stats$pct_eq0_05,
    alpha_eq_1_pct = alpha_stats$pct_eq1,
    lamrat_eq_0_125_pct = lamrat_stats$pct_eq0125,
    ...
    en_target_recovery_50pct = if (is.null(recovery_summary) || nrow(recovery_summary) == 0) NA_real_ else value_at_threshold(recovery_summary, 0.50, "en_any_pct"),
    dsen_shared_recovery_50pct = if (is.null(recovery_summary) || nrow(recovery_summary) == 0) NA_real_ else value_at_threshold(recovery_summary, 0.50, "dsen_shared_any_pct"),
    dsen_tissue_recovery_50pct = if (is.null(recovery_summary) || nrow(recovery_summary) == 0) NA_real_ else value_at_threshold(recovery_summary, 0.50, "dsen_tissue_any_pct"),
    dsen_combined_recovery_50pct = if (is.null(recovery_summary) || nrow(recovery_summary) == 0) NA_real_ else value_at_threshold(recovery_summary, 0.50, "dsen_combined_any_pct"),
    jaccard_en_vs_dsen_shared = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "jaccard_en_vs_dsen_shared_mean"),
    jaccard_en_vs_dsen_combined = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "jaccard_en_vs_dsen_combined_mean"),
    jaccard_dsen_shared_vs_tissue = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "jaccard_dsen_shared_vs_tissue_mean"),
    mean_en_genes = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "mean_en_genes"),
    mean_dsen_shared_genes = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "mean_dsen_shared_genes"),
    mean_dsen_tissue_genes = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "mean_dsen_tissue_genes"),
    mean_dsen_combined_genes = if (is.null(jaccard_summary)) NA_real_ else lookup_metric(jaccard_summary, "mean_dsen_combined_genes"),
...
metrics_to_column <- function(m) {
  c(
    ...
    format_pct(m$alpha_lt_0_2_pct, 1),
    format_pct(m$alpha_eq_0_05_pct, 1),
    format_pct(m$alpha_eq_1_pct, 1),
    format_pct(m$lamrat_eq_0_125_pct, 1),
...
v1_stage1 <- load_stage1_df(v1_stage1_rda)
v2_stage1 <- load_stage1_df(v2_stage1_rda)
uniform_stage1 <- if (include_uniform) load_stage1_df(uniform_stage1_rda) else NULL

v1_boot <- load_bootstrap_results(v1_bootstrap_rda)
v2_boot <- load_bootstrap_results(v2_bootstrap_rda)
uniform_boot <- if (include_uniform) load_bootstrap_results(uniform_bootstrap_rda) else NULL

# v2 detailed outputs
v2_recovery <- compute_recovery(v2_boot, targets_known, feature_info, recovery_thresholds)
v2_jaccard <- compute_jaccard(v2_boot, feature_info, jaccard_threshold)
v2_difficulty <- compute_difficulty_quartile(v2_stage1)

write_csv(v2_recovery$per_drug, out_v2_recovery_csv)
write_csv(v2_recovery$summary, out_v2_recovery_summary_csv)
write_csv(v2_jaccard$summary, out_v2_jaccard_csv)
write_csv(v2_jaccard$per_drug, out_v2_jaccard_per_drug_csv)
write_csv(v2_difficulty, out_v2_difficulty_csv)

tissue_target_recovery <- list(
  per_drug = v2_recovery$per_drug,
  summary = v2_recovery$summary,
  thresholds = recovery_thresholds,
  generated_at = Sys.time(),
  source = v2_label
)
save(tissue_target_recovery, file = out_v2_recovery_rda)

# Variant metrics
v1_recovery <- compute_recovery(v1_boot, targets_known, feature_info, recovery_thresholds)
v1_jaccard <- compute_jaccard(v1_boot, feature_info, jaccard_threshold)
v1_difficulty <- compute_difficulty_quartile(v1_stage1)

v1_metrics <- collect_metrics(v1_stage1, v1_recovery$summary, v1_jaccard$summary, v1_difficulty)
v2_metrics <- collect_metrics(v2_stage1, v2_recovery$summary, v2_jaccard$summary, v2_difficulty)
if (include_uniform) {
  uniform_recovery <- compute_recovery(uniform_boot, targets_known, feature_info, recovery_thresholds)
  uniform_jaccard <- compute_jaccard(uniform_boot, feature_info, jaccard_threshold)
  uniform_difficulty <- compute_difficulty_quartile(uniform_stage1)
  uniform_metrics <- collect_metrics(uniform_stage1, uniform_recovery$summary, uniform_jaccard$summary, uniform_difficulty)
}
...
comparison_tbl <- tibble(
  ...
  threshold_25_v1 = metrics_to_column(v1_metrics),
  threshold_25_v2 = metrics_to_column(v2_metrics)
)

if (include_uniform) {
  comparison_tbl$threshold_25_uniform <- metrics_to_column(uniform_metrics)
}

write_csv(comparison_tbl, out_comparison_csv)

cat("Wrote:\n")
cat("  ", out_v2_recovery_csv, "\n", sep = "")
cat("  ", out_v2_recovery_summary_csv, "\n", sep = "")
cat("  ", out_v2_jaccard_csv, "\n", sep = "")
cat("  ", out_v2_jaccard_per_drug_csv, "\n", sep = "")
cat("  ", out_v2_difficulty_csv, "\n", sep = "")
cat("  ", out_v2_recovery_rda, "\n", sep = "")
